...
def event_stream(conversation: list[dict]) -> str:
    ...
    for text_line in response:
        text = text_line.choices[0].delta.get('content', '')
        if text:
            yield text
...
if __name__ == "__main__":
    ...
    new_conversation = build_conversation_dict(messages=["Bonjour, comment ça va ?", "Ca va bien et toi ?"])
    for line in event_stream(new_conversation):
        print(line)
